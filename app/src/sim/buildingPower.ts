...
/** Net of every dwelling's live device draw, the building's own heat pump, AC, and
 * electric water heating (if any), and any rooftop solar (negative — a credit), at a
 * point in simulated time. `snowCoverCm` can be precomputed once by a caller scanning
 * many buildings at the same instant (see MapView's power-draw tick) rather than have
 * each one redo the same 30-day snow lookback; defaults to computing it fresh for a
 * single call. */
